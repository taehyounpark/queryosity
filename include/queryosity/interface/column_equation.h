...
template <typename Ret, typename... Vals>
template <typename F>
queryosity::column::equation<Ret(Vals...)>::equation(F callable)
    : m_evaluate(callable) {}

template <typename Ret, typename... Vals>
template <typename F, typename... Args>
queryosity::column::equation<Ret(Vals...)>::equation(F callable, Args &&...args)
    : m_evaluate(callable),
...
template <typename F>
auto queryosity::make_equation(F expression)
    -> std::unique_ptr<queryosity::column::template equation_t<F>> {
  return std::make_unique<queryosity::column::template equation_t<F>>(
      expression);
}