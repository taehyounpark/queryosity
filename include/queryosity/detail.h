...
#define CHECK_FOR_BINARY_OP(op_name, op_symbol)                                                                        \
    template <typename T, typename Arg, typename = void> struct has_native_##op_name : std::false_type                 \
    {                                                                                                                  \
    };                                                                                                                 \
                                                                                                                       \
    template <typename T, typename Arg>                                                                                \
    struct has_native_##op_name<T, Arg, std::void_t<decltype(std::declval<T>() op_symbol std::declval<Arg>())>>        \
        : std::true_type                                                                                               \
    {                                                                                                                  \
    };                                                                                                                 \
                                                                                                                       \
    template <typename T, typename Arg> auto operator op_symbol(const T &, const Arg &)->priority_tag<0>;              \
                                                                                                                       \
    template <typename T, typename Arg> struct has_custom_only_##op_name                                               \
    {                                                                                                                  \
      private:                                                                                                         \
        template <typename U, typename V>                                                                              \
        static auto test(priority_tag<1>) -> decltype(std::declval<U>() op_symbol std::declval<V>(),                   \
                                                      std::true_type());                                               \
                                                                                                                       \
        template <typename, typename> static auto test(priority_tag<0>) -> std::false_type;                            \
                                                                                                                       \
      public:                                                                                                          \
        static constexpr bool value =                                                                                  \
            decltype(test<T, Arg>(priority_tag<1>()))::value && !has_native_##op_name<T, Arg>::value;                  \
    };

#define DEFINE_LAZY_BINARY_OP(op_name, op_symbol)                                                                      \
    template <                                                                                                         \
        typename Arg, typename V = Action,                                                                             \
        typename std::enable_if<queryosity::is_column_v<V> && queryosity::is_column_v<typename Arg::action_type> &&    \
    ...
    {                                                                                                                  \
        return this->m_df->define(                                                                                     \
            queryosity::column::expression(                                                                            \
                [](column::value_t<V> const &me, column::value_t<typename Arg::action_type> const &you) {              \
...
#define DEFINE_LAZY_UNARY_OP(op_name, op_symbol)                                                                       \
    template <typename V = Action,                                                                                     \
              std::enable_if_t<queryosity::is_column_v<V> && detail::has_##op_name##_v<column::value_t<V>>, bool> =    \
                  false>                                                                                               \
    auto operator op_symbol() const                                                                                    \
    {                                                                                                                  \
        return this->m_df->define(                                                                                     \
            queryosity::column::expression([](column::value_t<V> const &me) { return (op_symbol me); }), *this);       \
    }
...
#define CHECK_FOR_INDEX_OP()                                                                                           \
    ...
    {                                                                                                                  \
        template <class T1, class IndexDeduced = Index,                                                                \
                  ...
                  typename = typename std::enable_if<!std::is_void<Reference>::value>::type>                           \
...
#define DEFINE_LAZY_INDEX_OP()                                                                                         \
    template <typename Arg, typename V = Action,                                                                       \
              std::enable_if_t<is_column_v<V> && detail::has_subscript_v<column::value_t<V>,                           \
    ...
    {                                                                                                                  \
        return this->m_df->define(                                                                                     \
            queryosity::column::expression(                                                                            \
                [](column::value_t<V> me, column::value_t<typename Arg::action_type> index) { return me[index]; }),    \
            *this, arg);                                                                                               \
    }

#define DECLARE_LAZY_VARIED_BINARY_OP(op_symbol)                                                                       \
    template <typename Arg, typename V = Act,                                                                          \
              std::enable_if_t<queryosity::is_column_v<V> && queryosity::is_column_v<typename Arg::action_type>,       \
                               bool> = false>                                                                          \
    auto operator op_symbol(Arg const &b) const->                                                                      \
...
#define DEFINE_LAZY_VARIED_BINARY_OP(op_symbol)                                                                        \
    ...
    template <                                                                                                         \
        ...
        std::enable_if_t<queryosity::is_column_v<V> && queryosity::is_column_v<typename Arg::action_type>, bool>>      \
...
#define DECLARE_LAZY_VARIED_UNARY_OP(op_symbol)                                                                        \
    template <typename V = Act, std::enable_if_t<queryosity::is_column_v<V>, bool> = false>                            \
    auto operator op_symbol() const->                                                                                  \
        typename lazy<typename decltype(std::declval<lazy<V>>().operator op_symbol())::action_type>::varied;
