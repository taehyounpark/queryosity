...
class dataflow
{
  ...
  public:
     ...
     *
     */
    template <typename Kwd1, typename Kwd2, typename Kwd3> dataflow(Kwd1 &&kwarg1, Kwd2 &&kwarg2, Kwd3 &&kwarg3);
     ...
     */
    template <typename DS> auto load(dataset::input<DS> &&in) -> dataset::loaded<DS>;
     ...
     */
    template <typename DS, typename Val> auto read(dataset::input<DS> in, dataset::column<Val> const &col);
     ...
     */
    template <typename DS, typename... Vals> auto read(dataset::input<DS> in, dataset::column<Vals> const &...cols);
     ...
     */
    template <typename Val> auto define(column::constant<Val> const &cnst) -> lazy<column::fixed<Val>>;
    ...
    auto define(column::expression<Fn> const &expr) -> lazy<column::equation_t<Fn>>;
     ...
     */
    template <typename Fn, typename... Cols>
    auto define(column::expression<Fn> const &expr, lazy<Cols> const &...cols) -> lazy<column::equation_t<Fn>>;
    ...
    auto define(column::definition<Def> const &defn, lazy<Cols> const &...cols) -> lazy<Def>;
     ...
     */
    template <typename Def, typename... Cols>
    auto define(column::definition<Def> const &defn, Cols const &...cols) -> typename lazy<Def>::varied;

    /**
     * @brief Select all entries.
     * @return Lazy selection with cut passing for all entries and weight equal to unity.
     */
     ...
     */
    template <typename Col> auto filter(lazy<Col> const &column) -> lazy<selection::node>;
     ...
     * @return Lazy selection.
     */
    template <typename Col> auto weight(lazy<Col> const &column) -> lazy<selection::node>;
     ...
     * @return Lazy (varied) selection.
     */
    template <typename Fn, typename... Cols> auto filter(column::constant<Fn> const &expr) -> lazy<selection::node>;
     ...
     * @return Lazy (varied) selection.
     */
    template <typename Val> auto weight(column::constant<Val> const &expr) -> lazy<selection::node>;
     ...
     * @return Lazy (varied) selection.
     */
    template <typename Fn, typename... Cols> auto filter(column::expression<Fn> const &expr, Cols const &...cols);
     ...
     * @return Lazy (varied) selection.
     */
    template <typename Fn, typename... Cols> auto weight(column::expression<Fn> const &expr, Cols const &...cols);
    ...
    auto _convert(lazy<Col> const &col) -> lazy<column::conversion<To, column::value_t<Col>>>;

    template <typename Val> auto _assign(Val const &val) -> lazy<column::fixed<Val>>;
    ...
    auto _equate(column::expression<Fn> const &expr) -> todo<column::evaluator<column::equation_t<Fn>>>;

    template <typename Sel, typename Col> auto _select(lazy<Col> const &col) -> lazy<selection::node>;

    template <typename Sel, typename Col>
    auto _select(lazy<selection::node> const &prev, lazy<Col> const &col) -> lazy<selection::node>;

    template <typename Qry, typename... Args> auto _make(Args &&...args) -> todo<query::booker<Qry>>;
  ...
  protected:
    ...
    auto _read(dataset::reader<DS> &ds, const std::string &name) -> lazy<read_column_t<DS, Val>>;

    template <typename Def, typename... Cols>
    auto _evaluate(todo<column::evaluator<Def>> const &calc, lazy<Cols> const &...columns) -> lazy<Def>;

    template <typename Qry>
    auto _book(todo<query::booker<Qry>> const &bkr, lazy<selection::node> const &sel) -> lazy<Qry>;
    template <typename Qry, typename... Sels>
    auto _book(todo<query::booker<Qry>> const &bkr, lazy<Sels> const &...sels) -> std::array<lazy<Qry>, sizeof...(Sels)>;

    template <typename Syst, typename Val>
    void _vary(Syst &syst, const std::string &name, column::constant<Val> const &cnst);

    template <typename Syst, typename Fn>
    void _vary(Syst &syst, const std::string &name, column::expression<Fn> const &expr);

    template <typename Syst, typename Defn>
    void _vary(Syst &syst, const std::string &name, column::definition<Defn> const &defn);
...
inline queryosity::dataflow::dataflow()
    : m_processor(multithread::disable()), m_weight(1.0), m_nrows(-1), m_all(nullptr), m_analyzed(false)
{
}
...
template <typename Kwd1, typename Kwd2> queryosity::dataflow::dataflow(Kwd1 &&kwarg1, Kwd2 &&kwarg2) : dataflow()
{
...
queryosity::dataflow::dataflow(Kwd1 &&kwarg1, Kwd2 &&kwarg2, Kwd3 &&kwarg3) : dataflow()
{
    static_assert(!std::is_same_v<Kwd1, Kwd2>, "repeated keyword arguments.");
...
template <typename Kwd> void queryosity::dataflow::accept_kwarg(Kwd &&kwarg)
{
    ...
    {
        static_assert(is_mt_v || is_weight_v || is_nrows_v, "unrecognized keyword argument");
    }
}

template <typename DS>
auto queryosity::dataflow::load(queryosity::dataset::input<DS> &&in) -> queryosity::dataset::loaded<DS>
{
...
template <typename DS, typename Val>
auto queryosity::dataflow::read(queryosity::dataset::input<DS> in, queryosity::dataset::column<Val> const &col)
{
...
auto queryosity::dataflow::read(queryosity::dataset::input<DS> in, queryosity::dataset::column<Vals> const &...cols)
{
    auto ds = this->load<DS>(std::move(in));
    return ds.read(cols...);
}
...
template <typename Col> auto queryosity::dataflow::filter(lazy<Col> const &col) -> lazy<selection::node>
{
    return this->_select<selection::cut>(col);
}

template <typename Col> auto queryosity::dataflow::weight(lazy<Col> const &col) -> lazy<selection::node>
{
...
auto queryosity::dataflow::all() -> lazy<selection::node>
{
    if (!m_all)
    {
        m_all = std::make_unique<lazy<selection::node>>(this->filter(column::constant(true)));
    }
    return *m_all;
}

template <typename Fn, typename... Cols>
auto queryosity::dataflow::filter(column::constant<Fn> const &cnst) -> lazy<selection::node>
{
    return this->filter(this->define(cnst));
}

template <typename Fn, typename... Cols>
auto queryosity::dataflow::filter(column::expression<Fn> const &expr, Cols const &...cols)
{
    return this->filter(this->define(expr, cols...));
}

template <typename Val> auto queryosity::dataflow::weight(column::constant<Val> const &cnst) -> lazy<selection::node>
{
    return this->weight(this->define(cnst));
}

template <typename Fn, typename... Cols>
auto queryosity::dataflow::weight(column::expression<Fn> const &expr, Cols const &...cols)
{
    return this->weight(this->define(expr, cols...));
}

template <typename Qry, typename... Args> auto queryosity::dataflow::_make(Args &&...args) -> todo<query::booker<Qry>>
{
    return todo<query::booker<Qry>>(
        *this, ensemble::invoke(
                   [&args...](dataset::player *plyr) { return plyr->template make<Qry>(std::forward<Args>(args)...); },
                   m_processor.get_slots()));
}

template <typename Qry>
auto queryosity::dataflow::make(queryosity::query::plan<Qry> const &cntr) -> todo<query::booker<Qry>>
{
...
{
    auto act = ensemble::invoke([](dataset::player *plyr, column::evaluator<Def> *calc,
                                   Cols const *...cols) { return plyr->template evaluate(*calc, *cols...); },
...
template <typename Qry>
auto queryosity::dataflow::_book(todo<query::booker<Qry>> const &bkr, lazy<selection::node> const &sel) -> lazy<Qry>
{
    // new query booked: dataset will need to be analyzed
    this->reset();
    auto act = ensemble::invoke(
        [](dataset::player *plyr, query::booker<Qry> *bkr, const selection::node *sel) { return plyr->book(*bkr, *sel); },
...
auto queryosity::dataflow::_book(todo<query::booker<Qry>> const &bkr,
                                 lazy<Sels> const &...sels) -> std::array<lazy<Qry>, sizeof...(Sels)>
{
...
inline void queryosity::dataflow::reset()
{
    m_analyzed = false;
}
...
auto queryosity::dataflow::vary(column::definition<Defn> const &defn, Vars const &...vars)
{
    ...
    using varied_type = typename decltype(nom)::varied;
    varied_type syst(std::move(nom));
    ((this->_vary(syst, vars.name(), vars.template _vary_arg<column::definition<Defn>>())), ...);
    return syst;
}

template <typename DS, typename Val>
auto queryosity::dataflow::_read(dataset::reader<DS> &ds,
                                 const std::string &column_name) -> lazy<read_column_t<DS, Val>>
{
...
template <typename Fn> auto queryosity::dataflow::_equate(Fn fn)
{
    return todo<column::evaluator<typename column::equation_t<Fn>>>(
        *this,
        ensemble::invoke([fn](dataset::player *plyr) { return plyr->template equate(fn); }, m_processor.get_slots()));
}
...
template <typename Sel, typename Col> auto queryosity::dataflow::_select(lazy<Col> const &dec) -> lazy<selection::node>
{
    auto act =
        ensemble::invoke([](dataset::player *plyr, Col *col) { return plyr->template select<Sel>(nullptr, *col); },
...
template <typename Sel, typename Col>
auto queryosity::dataflow::_select(lazy<selection::node> const &prev, lazy<Col> const &dec) -> lazy<selection::node>
{
    auto act = ensemble::invoke(
        [](dataset::player *plyr, selection::node *prev, Col *col) { return plyr->template select<Sel>(prev, *col); },
...
template <typename Syst, typename Val>
void queryosity::dataflow::_vary(Syst &syst, const std::string &name, column::constant<Val> const &cnst)
{
    syst.set_variation(name, this->define(cnst));
}

template <typename Syst, typename Fn>
void queryosity::dataflow::_vary(Syst &syst, const std::string &name, column::expression<Fn> const &expr)
{
    syst.set_variation(name, this->_equate(expr));
}

template <typename Syst, typename Defn>
void queryosity::dataflow::_vary(Syst &syst, const std::string &name, column::definition<Defn> const &defn)
{
