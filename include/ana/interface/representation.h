...
public:
  ...
  constexpr auto value() const -> decltype(std::get<N>(std::declval<vartuple_type>()).value());
...
class column::representation<Ret(Obs...)> : public term<Ret>::template representation_of<Obs...>
{
...
template <typename... Obs>
template <typename... Vals>
void ana::term<Ret>::representation_of<Obs...>::set_arguments(cell<Vals> const&... args)
{
...
template <typename... Obs>
template <unsigned int N>
constexpr auto ana::term<Ret>::representation_of<Obs...>::value() const -> decltype(std::get<N>(std::declval<vartuple_type>()).value())
{
