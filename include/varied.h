...
// lazy<T> -> varied<T> seems like a headche, but
// all of these are actually quite straightforward now.
// we just apply whatever the original lazy action is doing,
// simply to the nominal+variations.

// starting with mathematical operations,
// the nightmare type_traits gymnastics is no more,
// we just do whatever lazy has available
#define DECLARE_VARIED_BINARY_OP(op_symbol)\
template <typename Arg>\
auto operator op_symbol(Arg&& b) const  -> varied<typename decltype(std::declval<lazy<Act>>().operator op_symbol(std::forward<Arg>(b).nominal()))::action_type>;
#define DEFINE_VARIED_BINARY_OP(op_symbol)\
template <typename T>\
template <typename Act>\
template <typename Arg>\
auto ana::analysis<T>::varied<Act>::operator op_symbol(Arg&& b) const  -> varied<typename decltype(std::declval<lazy<Act>>().operator  op_symbol(std::forward<Arg>(b).nominal()))::action_type>\
{\
	auto syst = varied<typename decltype(std::declval<lazy<Act>>().operator  op_symbol(std::forward<Arg>(b).nominal()))::action_type>(nominal().operator op_symbol(std::forward<Arg>(b).nominal()));\
...
{\
	auto syst = varied<typename decltype(std::declval<lazy<V>>().operator  op_symbol())::action_type>(nominal().operator op_symbol());\
...
public:
	...
	virtual lazy<Act> nominal() const override;
	virtual lazy<Act> variation(const std::string& var_name) const override;
  ...
  auto channel(const std::string& name) -> varied<simple_selection_evaluator_type>;
	...
	template <typename... Nodes, typename V = Act, typename std::enable_if_t<ana::is_counter_booker_v<V>, V>* = nullptr>
	auto at(Nodes const&... selections) -> varied<typename decltype(std::declval<lazy<V>>().at(selections.nominal()...))::action_type>;

	template <typename... Args>
	auto operator()(Args&&... args) -> varied<typename decltype(std::declval<lazy<Act>>().operator()(std::forward<Args>(args).nominal()...))::action_type>;
...
template <typename T>
template <typename Act>
auto ana::analysis<T>::varied<Act>::nominal() const -> lazy<Act>
{
	return m_nominal;
}
...
template <typename T>
template <typename Act>
auto ana::analysis<T>::varied<Act>::variation(const std::string& var_name) const -> lazy<Act>
{
...
template <typename Act>
template <typename... Nodes, typename V, typename std::enable_if_t<ana::is_counter_booker_v<V>, V>* ptr>
auto ana::analysis<T>::varied<Act>::at(Nodes const&... selections) -> varied<typename decltype(std::declval<lazy<V>>().at(selections.nominal()...))::action_type>
// varied version of booking counter at a selection operation
{
	varied<typename decltype(std::declval<lazy<V>>().at(selections.nominal()...))::action_type> syst(nominal().at(selections.nominal()...));
...
template <typename Act>
template <typename... Args>
auto ana::analysis<T>::varied<Act>::operator()(Args&&... args) -> varied<typename decltype(std::declval<lazy<Act>>().operator()(std::forward<Args>(args).nominal()...))::action_type>
{
	auto syst = varied<typename decltype(std::declval<lazy<Act>>().operator()(std::forward<Args>(args).nominal()...))::action_type>(nominal().operator()(std::forward<Args>(args).nominal()...));
	for (auto const& var_name : list_all_variation_names(*this, std::forward<Args>(args)...)) {
