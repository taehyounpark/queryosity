...
template <typename T>
class processor : public action, public column::computation<T>, public counter::experiment
{
...
ana::processor<T>::processor(const input::range& part, input::reader<T>& reader, double scale) :
  action(),
	column::computation<T>(part,reader),
...
void ana::processor<T>::process()
{
	...
	// per-entry
	for (unsigned long long entry=this->m_part.begin; entry<this->m_part.end ; ++entry) {
