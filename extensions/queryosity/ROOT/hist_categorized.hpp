...
class hist_categorized
    : public query::definition<std::map<Cat, std::shared_ptr<TH1>>(Cat, Val)> {
...
public:
  ...
  merge(std::vector<std::map<Cat, std::shared_ptr<TH1>>> const &results)
...
template <typename Cat, typename Val>
queryosity::ROOT::hist_categorized<Cat, Val>::hist_categorized(
    const std::string &hname, std::vector<Cat> const &categories,
...
template <typename Cat, typename Val>
void queryosity::ROOT::hist_categorized<Cat, Val>::fill(
    column::observable<Cat> cat, column::observable<Val> x, double w) {
  m_hists[cat.value()]->Fill(x.value(), w);
}

template <typename Cat, typename Val>
std::map<Cat, std::shared_ptr<TH1>>
queryosity::ROOT::hist_categorized<Cat, Val>::result() const {
  return m_hists;
}

template <typename Cat, typename Val>
std::map<Cat, std::shared_ptr<TH1>>
queryosity::ROOT::hist_categorized<Cat, Val>::merge(
  ...
  for (auto const &[cat, hist] : results[0]) {
    merged_result[cat] =
        std::shared_ptr<TH1>(static_cast<TH1 *>(hist->Clone()));
